...
namespace MyEshop.Areas.Admin.Controllers
{
    public class Product_GroupsController : Controller
    {
        ...
        {
            var product_Groups = db.Product_Groups.Where(g => g.ParentID == null).ToList();
            return PartialView(product_Groups);
        }

        public string nameGroup(int id)
        {
            return db.Product_Groups.Find(id).ToString();
        ...
        public ActionResult Create([Bind(Include = "GroupID,GroupTitle,ParentID")] Product_Groups product_Groups)
        {
            if (ModelState.IsValid)
            {
                ...
                db.SaveChanges();
                return PartialView("ListGroups", db.Product_Groups.Where(g => g.ParentID == null).ToList());
            }
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            {
                foreach (var SubGroup in product_Groups.Product_Groups1)
                {
                    db.Entry(SubGroup).State = EntityState.Modified;
                }
            }
